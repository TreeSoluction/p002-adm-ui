...
export default function Page({ params }: any) {
  ...
  useEffect(() => {
    const loadData = async () => {
      const result = await apiGet<any>(`/freteiros/${params.id}`);
      setForm({
        nome: result.nome || "",
        local: result.local || "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (hasValidId(params.id)) {
        await apiPut<any>(`/freteiros/${params.id}`, form);
      } else {
        await apiPost<any>("/freteiros", form);
      }
      router.push("/dashboard/freteiros");
    } catch (e) {
      console.error(e);
    }
  };
  ...
  return (
    <div className="max-w-xl mx-auto bg-white p-8 rounded-lg shadow text-gray-800">
      <h1 className="text-2xl font-bold mb-6">
        {hasValidId(params.id) ? "Editar freteiros" : "Nova freteiros"}
      </h1>
