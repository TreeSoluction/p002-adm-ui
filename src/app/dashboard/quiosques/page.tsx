...
export default function quiosquesPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      const response = await apiGet<any>(
        `${API_ROUTES.QUIOSQUES}?${params.toString()}`
      );
  ...
  const handleEdit = (id: any) => {
    router.push(`/dashboard/quiosques/${id}`);
  };
  ...
  return (
    <div className="flex flex-col gap-2">
      <h1 className="text-2xl font-bold text-blue-700 mb-6">Meus Quiosques</h1>
      <div className="flex justify-between gap-10">
        ...
        <button
          ...
          onClick={() => router.push("/dashboard/quiosques/gerenciar")}
        >
          Criar
        </button>
      </div>
