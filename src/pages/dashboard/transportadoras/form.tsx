...
export default function TransportadorasFormPage() {
  const navigate = useNavigate(); const { id } = useParams();
  const [form, setForm] = useState<{ nome: string; local: string; cidade: string; imagem: string; email: string; whatsapp: string; instagram: string; phone_numbers: string[]; produtos: string[]; }>({ nome: "", local: "", cidade: "", imagem: "", email: "", whatsapp: "", instagram: "", phone_numbers: [""], produtos: [] });
  const [cidades, setCidades] = useState<any[]>([]);

  useEffect(() => {
    const loadData = async () => { if (hasValidId(id)) { const result = await apiGet<any>(`/transportadoras/${id}`); setForm({ nome: result.nome || "", local: result.local || "", cidade: result.cidade || "", imagem: result.imagem || "", email: result.email || "", whatsapp: result.whatsapp || "", instagram: result.instagram || "", phone_numbers: result.phone_numbers?.length > 0 ? result.phone_numbers : [""], produtos: result.produtos || [] }); } };
  ...
  const handleSubmit = async (e: React.FormEvent) => { e.preventDefault(); try { if (hasValidId(id)) { await apiPut<any>(`/transportadoras/${id}`, form); } else { await apiPost<any>("/transportadoras", form); } navigate("/dashboard/transportadoras"); } catch (e) { console.error(e); } };

  return (
    <div className="max-w-xl mx-auto bg-white p-8 rounded-lg shadow text-gray-800">
      <h1 className="text-2xl font-bold mb-6">{hasValidId(id) ? "Editar Transportadora" : "Nova Transportadora"}</h1>
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div><label className="block font-semibold mb-1">Email</label><input type="email" name="email" value={form.email} onChange={handleChange} className="w-full border border-blue-200 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-400" required /></div>
