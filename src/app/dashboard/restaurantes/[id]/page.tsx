...
export default function Page({ params }: any) {
  ...
  useEffect(() => {
    const loadData = async () => {
      if (hasValidId(params.id)) {
        const result = await apiGet<any>(`/restaurantes/${params.id}`);
        setForm({
          nome: result.nome || "",
          local: result.local || "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (hasValidId(params.id)) {
        await apiPut<any>(`/restaurantes/${params.id}`, form);
      } else {
        await apiPost<any>("/restaurantes", form);
      }
  ...
  return (
    <div className="max-w-xl mx-auto bg-white p-8 rounded-lg shadow text-gray-800">
      <h1 className="text-2xl font-bold mb-6">
        {hasValidId(params.id) ? "Editar Restaurante" : "Nova Restaurante"}
      </h1>
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <input
            ...
            onChange={handleChange}
            className="w-full border border-blue-200 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-400"
          />
        </div>
        <div>
