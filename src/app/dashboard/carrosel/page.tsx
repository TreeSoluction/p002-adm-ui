...
const columns = [
  { key: "id", label: "ID" },
  { key: "imagem", label: "Imagem" }, // Agora é só "imagem"
];
...
export default function Page() {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      const response = await apiGet<any>(
        `${API_ROUTES.CARROSEL}?${params.toString()}`
      );
  ...
  const handleEdit = (id: any) => {
    router.push(`/dashboard/carrosel/${id}`);
  };
  ...
  return (
    <div className="flex flex-col gap-2">
      <h1 className="text-2xl font-bold text-blue-700 mb-6">Minhas Imagens</h1>
      <div className="flex justify-end gap-10">
        <button
          className="flex bg-blue-800 p-1 px-10 rounded-sm justify-center items-center"
          onClick={() => router.push("/dashboard/carrosel/gerenciar")}
        >
          Adicionar Imagem
        </button>
      </div>
