...
export default function freteirosPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      const response = await apiGet<any>(
        `${API_ROUTES.FRETEIROS}?${params.toString()}`
      );
  ...
  const handleEdit = (id: any) => {
    router.push(`/dashboard/freteiros/${id}`);
  };
    ...
    <div className="flex flex-col gap-2">
      ...
      <div className="flex justify-between gap-10">
        ...
        <button
          ...
          onClick={() => router.push("/dashboard/freteiros/gerenciar")}
        >
          Criar
        </button>
      </div>
