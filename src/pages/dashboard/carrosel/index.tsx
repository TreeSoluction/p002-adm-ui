...
export default function Page() {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      const response = await apiGet<any>(`${API_ROUTES.CARROSEL}?${params.toString()}`);
  ...
  const handleDelete = async (id: any) => {
    await apiDelete<any>(`${API_ROUTES.CARROSEL}/${id.toString()}`);
    setData((prev) => prev.filter((item) => item.id !== id));
  };

  const handleEdit = (id: any) => navigate(`/dashboard/carrosel/${id}`);
      ...
      <div className="overflow-x-auto bg-white rounded-lg shadow">
        <table className="min-w-full divide-y divide-blue-200">
          ...
          <tbody className="bg-white divide-y divide-blue-100 text-black">
            ...
            {sortedData.length === 0 && (<tr><td colSpan={columns.length + 1} className="text-center py-6 text-blue-400">Nenhuma imagem encontrada.</td></tr>)}
          </tbody>
        </table>
      </div>
