...
export default function RestaurantesFormPage() {
  ...
  useEffect(() => {
    const loadData = async () => { if (hasValidId(id)) { const result = await apiGet<any>(`/restaurantes/${id}`); setForm({ nome: result.nome || "", local: result.local || "", cidade: result.cidade || "", imagem: result.imagem || "", whatsapp: result.whatsapp || "", instagram: result.instagram || "", phone_numbers: result.phone_numbers?.length > 0 ? result.phone_numbers : [""], produtos: result.produtos || [] }); } };
  ...
  const handleSubmit = async (e: React.FormEvent) => { e.preventDefault(); try { if (hasValidId(id)) { await apiPut<any>(`/restaurantes/${id}`, form); } else { await apiPost<any>("/restaurantes", form); } navigate("/dashboard/restaurantes"); } catch (e) { console.error(e); } };

  return (
    <div className="max-w-xl mx-auto bg-white p-8 rounded-lg shadow text-gray-800">
      <h1 className="text-2xl font-bold mb-6">{hasValidId(id) ? "Editar Restaurante" : "Novo Restaurante"}</h1>
