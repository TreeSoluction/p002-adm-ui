...
export default function HospedagensFormPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      if (hasValidId(id)) {
        const result = await apiGet<any>(`/hospedagens/${id}`);
        setForm({ nome: result.nome || "", local: result.local || "", cidade: result.cidade || "", imagem: result.imagem || "", whatsapp: result.whatsapp || "", instagram: result.instagram || "", phone_numbers: result.phone_numbers?.length > 0 ? result.phone_numbers : [""], produtos: result.produtos || [] });
      }
    };
    loadData();
    apiGet<any>("/cidades?size=1000&page=0").then((r) => setCidades(r.data)).catch(console.error);
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try { if (hasValidId(id)) { await apiPut<any>(`/hospedagens/${id}`, form); } else { await apiPost<any>("/hospedagens", form); } navigate("/dashboard/hospedagens"); } catch (e) { console.error(e); }
  };

  return (
    <div className="max-w-xl mx-auto bg-white p-8 rounded-lg shadow text-gray-800">
      <h1 className="text-2xl font-bold mb-6">{hasValidId(id) ? "Editar Hospedagem" : "Nova Hospedagem"}</h1>
